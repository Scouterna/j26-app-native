...
extension ViewController: WKUIDelegate, WKDownloadDelegate {
    // Handle geolocation permission requests from the web page
    func webView(_ webView: WKWebView, requestGeolocationPermissionFor origin: WKSecurityOrigin, initiatedByFrame frame: WKFrameInfo, decisionHandler: @escaping (WKPermissionDecision) -> Void) {
        let requestingDomain = origin.host
        let isAllowed = allowedOrigins.contains(where: { requestingDomain.range(of: $0) != nil })
            || authOrigins.contains(where: { requestingDomain.range(of: $0) != nil })
